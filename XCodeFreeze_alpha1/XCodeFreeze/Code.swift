...
struct MCPConfig: Codable {
    ...
    static func loadConfig() throws -> MCPConfig {
        // Use explicit full path to mcp.json
        let configPath = "/Users/toddbruss/.cursor/mcp.json"
        print(configPath)
        
        print("Attempting to load MCP config from absolute path: \(configPath)")
        
        // Check if file exists before trying to read it
        let fileManager = FileManager.default
        guard fileManager.fileExists(atPath: configPath) else {
            print("Config file does not exist at absolute path: \(configPath)")
            
            throw NSError(domain: "MCPConfig", code: 404, userInfo: [NSLocalizedDescriptionKey: "Config file not found"])
        }
        ...
        if !config.mcpServers.isEmpty {
            ...
            if let firstServerName = config.mcpServers.keys.first {
                ...
                print("Setting MCP_SERVER_NAME to: \(firstServerName) (from config)")
            }
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func debugPrintMCPConfigFile() {
        do {
            // Use explicit full path to mcp.json
            let configPath = "/Users/TBruss/.cursor/mcp.json"
            
            print("Checking for MCP config at absolute path: \(configPath)")
            ...
            if fileManager.fileExists(atPath: configPath) {
                ...
                print("MCP config file does not exist at absolute path: \(configPath)")
            }
        } catch {
...
class MCPViewModel: ObservableObject {
    ...
    func startClientServer() async {
        ...
        do {
            ...
            // First try to find configuration in ~/.cursor/mcp.json
            var useBuiltInServer = true
            var serverCommandComponents: [String] = []
            
            do {
                let config = try MCPConfig.loadConfig()
    ...
    private func discoverParametersFromToolSchemas() async {
        ...
        for tool in self.availableTools {
            do {
                ...
                print("Error discovering schema for tool \(tool.name): \(error)")
            }
        }
        
        if discoveredSchemaCount > 0 {
            await addMessage(content: "Discovered schemas for \(discoveredSchemaCount) tools.", isFromServer: true)
        }
    }
    ...
    private func processHelpOutput(_ helpText: String) async {
        ...
        if !serverActions.isEmpty {
            ...
            // Make a local copy that's captured by the closure instead of the original variable
            let actionsToAdd = Array(serverActions)
            let descriptionsToAdd = actionDescriptions
            
            // Update the available tools display
            await MainActor.run {
                // Add all the server actions as tools to the UI
                for action in actionsToAdd {
                    if !self.availableTools.contains(where: { $0.name == action }) {
                        let description = descriptionsToAdd[action] ?? "\(MCP_SERVER_NAME) action"
                        self.availableTools.append(MCPTool(name: action, description: description))
                    }
                }
            }
        } else {
            ...
            await MainActor.run {
                for tool in tools {
                    ...
                    if let index = self.availableTools.firstIndex(where: { $0.name == tool }),
                       let betterDescription = toolDescriptions[tool],
                    ...
                    // If the tool is not in our list yet
                        ...
                        let description = toolDescriptions[tool] ?? tool
