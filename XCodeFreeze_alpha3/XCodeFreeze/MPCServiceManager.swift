...
class MCPServiceManager {
    ...
    var messageCallback: (String, Bool) -> Void
    var onConnectionEstablished: ([MCPTool]) async -> Void = { _ in }
    
    init(statusCallback: @escaping (String) -> Void, messageCallback: @escaping (String, Bool) -> Void) {
        self.statusCallback = statusCallback
        self.messageCallback = messageCallback
    }
    
    func startServer(configPath: String? = nil) async {
        messageCallback("Starting XCodeFreeze Client Demo...", true)
        
        if let customPath = configPath, !customPath.isEmpty {
            messageCallback("Using custom config file: \(customPath)", true)
        }
        ...
        do {
            ...
            do {
                ...
                if let serverConfig = config.mcpServers[MCP_SERVER_NAME] {
                    ...
                    if serverType == "stdio" {
                        ...
                        statusCallback("Found \(MCP_SERVER_NAME) server configuration")
                        messageCallback("Found \(MCP_SERVER_NAME) server: \(executablePath)", true)
                        ...
                        Task {
                            ...
                            if !errorData.isEmpty, let errorString = String(data: errorData, encoding: .utf8) {
                                ...
                                messageCallback("\(MCP_SERVER_NAME) Server error: \(errorString)", true)
                            }
                        }
                        ...
                        do {
                            ...
                            messageCallback(errorMessage, true)
                            return
                        }
                        
                        statusCallback("\(MCP_SERVER_NAME) Server started, connecting client...")
                        ...
                        do {
                            ...
                            guard let pingData = pingMessage.data(using: .utf8) else {
                                statusCallback("Error: Failed to encode ping message")
                                messageCallback("Error: Failed to encode ping message", true)
                                return
                            }
                            ...
                            statusCallback("Error connecting to \(MCP_SERVER_NAME) server: \(error.localizedDescription)")
                            messageCallback("Error connecting to \(MCP_SERVER_NAME) server: \(error.localizedDescription)", true)
                        ...
                        messageCallback("\(MCP_SERVER_NAME) server type \(serverType) is not supported, using built-in server", true)
                    }
                } else {
                    messageCallback("Error: No server configuration found for \(MCP_SERVER_NAME) in config file.", true)
                    statusCallback("No server configuration found")
                    return
                }
            } catch {
                print("Error loading MCP config: \(error.localizedDescription)")
                messageCallback("Error loading config: \(error.localizedDescription)", true)
                statusCallback("Config error: \(error.localizedDescription)")
                return
            }
            
            // Verify that client was created successfully
            guard client != nil && clientCreated else {
                messageCallback("Error: Failed to create MCP client", true)
                statusCallback("Client creation failed")
                return
            }
            ...
            messageCallback("[→] JSON-RPC: initialize(clientName: \"\(MCP_CLIENT_NAME)\", version: \"\(MCP_CLIENT_DEFAULT_VERSION)\")", true)
            
            // Initialize connection
            guard let client = self.client else {
                statusCallback("Error: Client was not created properly")
                messageCallback("Error: Failed to create client object", true)
                return
            }
            ...
            messageCallback("[←] JSON-RPC Response: { \"serverInfo\": { \"name\": \"\(serverName)\", \"version\": \"\(serverVersion)\" } }", true)
            ...
            if !serverName.isEmpty {
                ...
                if serverName.lowercased() == "filesystem" && MCP_SERVER_NAME.lowercased() != "filesystem" {
                    messageCallback("Connected to NPX Filesystem server. Note: Using configured name '\(MCP_SERVER_NAME)' for server commands.", true)
            ...
            statusCallback("Connected to: \(serverName) v\(serverVersion)")
            messageCallback("Connected to server: \(serverName) v\(serverVersion)", true)
            
            // Add JSON-RPC debug message for listTools
            messageCallback("[→] JSON-RPC: listTools()", true)
            ...
            for tool in toolsResponse.tools {
                ...
                if let inputSchema = tool.inputSchema {
                    ...
                    extractParametersFromSchema(toolName: tool.name, schema: inputSchema)
                }
            ...
            messageCallback("[←] JSON-RPC Response: { \"tools\": \(toolsJson) }", true)
            
            isConnected = true
            
            let availableTools = toolsResponse.tools.map {
                MCPTool(name: $0.name, description: $0.description)
            }
            
            // Return the available tools through the callback
            messageCallback("Available tools: \(toolsResponse.tools.map { $0.name }.joined(separator: ", "))", true)
            
            // Notify ViewModel that connection is successful and pass the tools
            await onConnectionEstablished(availableTools)
            
            // Automatically discover available tools and parameters
            await discoverToolsAndParameters(availableTools)
            
        } catch {
            statusCallback("Error: \(error.localizedDescription)")
            messageCallback("Error: \(error.localizedDescription)", true)
        }
    }
    
    // Automatically discover tools and their parameters
    private func discoverToolsAndParameters(_ tools: [MCPTool]) async {
        ...
        if tools.contains(where: { $0.name == "help" || $0.name == "mcp_\(MCP_SERVER_NAME)_help" }) {
            messageCallback("Querying for available \(MCP_SERVER_NAME) actions...", true)
            
            // Try with mcp_xcf_help first, then fall back to help
            if tools.contains(where: { $0.name == "mcp_\(MCP_SERVER_NAME)_help" }) {
                await callTool(name: "mcp_\(MCP_SERVER_NAME)_help", text: "")
            } else if tools.contains(where: { $0.name == "help" }) {
                await callTool(name: "help", text: "")
            }
        }
        
        // Finally, query for tool list to understand available tools
        if tools.contains(where: { $0.name == "list" || $0.name == "mcp_\(MCP_SERVER_NAME)_list" }) {
            messageCallback("Querying for available tools...", true)
            
            // Try with mcp_xcf_list first, then fall back to list
            if tools.contains(where: { $0.name == "mcp_\(MCP_SERVER_NAME)_list" }) {
                await callTool(name: "mcp_\(MCP_SERVER_NAME)_list", text: "")
            } else if tools.contains(where: { $0.name == "list" }) {
        ...
        if let serverActions = ToolRegistry.shared.getSubTools(for: MCP_SERVER_NAME), !serverActions.isEmpty {
            messageCallback("You can use \(MCP_SERVER_NAME) actions by typing '\(MCP_SERVER_NAME) <action>' or just the action name directly.", true)
        }
    }
    ...
    func callTool(name: String, text: String) async {
        ...
        messageCallback(displayText, false)
        
        guard let client = client, isConnected else {
            messageCallback("Error: Client not connected", true)
            return
        }
        ...
        """
        messageCallback(callToolJson, true)
        ...
        do {
            ...
            """
            messageCallback(responseJson, true)
            
            // Process response
            if isError == true {
                messageCallback("Error from server", true)
                return
            }
            ...
            for item in content {
                if case .text(let responseText) = item {
                    messageCallback("Result: \(responseText)", true)
    ...
    private func processHelpOutput(_ helpText: String) async {
        ...
        if !serverActions.isEmpty {
            ...
            messageCallback("Available \(MCP_SERVER_NAME) actions: \(actionNames)", true)
    ...
    func getDiagnostics() async {
        guard isConnected else {
            messageCallback("[DIAGNOSTICS] Client is not connected", true)
            return
        }
        
        messageCallback("[DIAGNOSTICS] Checking client-server connection...", true)
        
        // Check transport
        if let _ = transport {
            messageCallback("[DIAGNOSTICS] Transport exists", true)
        } else {
            messageCallback("[DIAGNOSTICS] Transport is nil", true)
        }
        
        // Check client
        if let client = client {
            messageCallback("[DIAGNOSTICS] Client exists (\(client.name) v\(client.version))", true)
        } else {
            messageCallback("[DIAGNOSTICS] Client is nil", true)
        }
        
        // Check server process
        if let process = serverProcess {
            let isRunning = process.isRunning
            messageCallback("[DIAGNOSTICS] Server process exists (PID: \(process.processIdentifier), Running: \(isRunning))", true)
        } else {
            messageCallback("[DIAGNOSTICS] Server process is nil", true)
        }
        
        // Test client with a ping if available
        if let client = client {
            messageCallback("[DIAGNOSTICS] Testing client with ping...", true)
            do {
                _ = try await client.ping()
                messageCallback("[DIAGNOSTICS] Ping successful", true)
            } catch {
                messageCallback("[DIAGNOSTICS] Ping failed: \(error.localizedDescription)", true)
            }
        }
    }
    ...
    func startDebugMessageTest() async {
        ...
        messageCallback("[DEBUG] Sending ping message to test communication", true)
        
        // Add JSON-RPC debug message
        messageCallback("[→] JSON-RPC: ping()", true)
        ...
        do {
            ...
            // Success response
            messageCallback("[←] JSON-RPC Response: { \"result\": {} }", true)
            messageCallback("[DEBUG] Ping successful! Communication with server is working.", true)
        } catch {
            messageCallback("[DEBUG] Error during ping test: \(error.localizedDescription)", true)
        }
    }
    
    // Improved parameter extraction from schemas
    private func extractParametersFromSchema(toolName: String, schema: Value) {
        ...
        if !parameterMap.isEmpty {
            ...
            messageCallback("Discovered parameters for \(toolName): \(paramNames)", true)
